					...
					<table class="table table-hover overflow-auto block">
						...
						<tbody>
							...
							<tr>
								...
								<td>
									<a href="{{ route('testimonial.edit', $data->id) }}" class="badge badge-primary cursor-pointer">Edit</a>
								</td>
								<td>
									@if ($isAdmin === true)
									<form method="POST" action="{{ route('testimonial.destroy', $data->id) }}">
										@method('DELETE')
										@csrf
										<button type="submit" class="badge badge-danger cursor-pointer" onclick="return confirmDeleteReview({{ $data->id }} , '{{ $data->name }}');">Delete</button>
									</form>
									@else
